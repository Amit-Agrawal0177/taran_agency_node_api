...
exports.productCruds = async (req, res) => {
  try {
    var { prod_id, prod_name, description, image, price, batch, mfg, cgst, sgst, is_active } = req.body;

    if(prod_id)
    {
      let updateData = await productQuery.updateRoductDetails(prod_id, { prod_name, description, image, price, batch, mfg, cgst, sgst, is_active });
    ...
    let updateData = await productQuery.insertProductDetails({ prod_name, description, image, price, batch, mfg, cgst, sgst });
